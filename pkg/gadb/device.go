...
func isRootSuccessful(line string) bool {
	return line == "* daemon started successfully *" || line == "adbd is already running as root"
}

func (d Device) Root() error {
retry:
	for attempts := 0; attempts < MaxRootAttempts; attempts++ {
		resp, err := d.executeCommand("root:", true)
		if err != nil {
			return fmt.Errorf("failed to root device: %w", err)
		}
		// Assume no output is success.
		if len(resp) == 0 {
			return nil
		}
	...
	return fmt.Errorf("failed to root device")
}
...
func (d Device) Logcat2File(file string, exitChan chan bool) error {
	f, err := os.OpenFile(file, os.O_WRONLY|os.O_CREATE|os.O_SYNC|os.O_APPEND, 0o755)
	if err != nil {
		return err
	}
	defer f.Close()
