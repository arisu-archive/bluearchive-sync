...
type CustomSerializer interface {
	SerializeObject(enc *Encoder) error
}
...
func (enc *Encoder) newObject(object interface{}) error {
	// Check if struct has custom serialization function first
	if customSerializer, hasCustom := object.(CustomSerializer); hasCustom {
		return customSerializer.SerializeObject(enc)
	}
